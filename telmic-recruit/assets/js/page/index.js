...
(function ($) {
     ...
     */
    $(function() {
        var $target = $('.index-page .news-article');
        var $button = $('.index-page .news-nav-button');
     ...
     */
    $(function () {
        var $article = $('.interview-row');
        var length = $article.length;
        $article.each(function (i) {
            $(this).find('.interview-nav-number')
                .text((i + 1) + ' / ' + length);
        });

        var slider = new Swiper('.interview-wrap', {
            loop: true
        });

        $(document).on('click', '.interview-nav-button', function (e) {
            e.preventDefault();
